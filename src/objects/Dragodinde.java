...
public class Dragodinde {
	...
	{
		_id = World.data.getNextIdForMount();
		_color = color;
		_level = 1;
		_exp = 0;
		_nom = "SansNom";
	...
	{
		return _exp+","+World.data.getExpLevel(_level).dinde+","+World.data.getExpLevel(_level+1).dinde;
	}
	...
	public void addXp(long amount)
	{
		...
		while(_exp >= World.data.getExpLevel(_level+1).dinde && _level<100)
			levelUp();
