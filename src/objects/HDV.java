...
public class HDV {
	...
	private class Template
	{
		...
		public void addEntry(HdvEntry toAdd)
		{
			...
			int ligneID = World.data.getNextLigneID();
	...
	{
		int type = World.data.getObjTemplate(templateID).getType();
	...
	public void addEntry(HdvEntry toAdd)
	{
		...
		World.data.addHdvItem(toAdd.getOwner(), _hdvID, toAdd);		
	}
	public boolean delEntry(HdvEntry toDel)
	{
		...
		if(toReturn)
		{
			...
			World.data.removeHdvItem(toDel.getOwner(), toDel.getHdvID(), toDel);
		}
	...
	public synchronized boolean buyItem(int ligneID,byte amount, int price, Personnage newOwner)
	{
		...
		try
		{
			...
			if(toBuy.getOwner() == -1)
			{
				World.database.getItemData().update(toBuy.getObjet());
			}
			toBuy = null;
		}
		catch(NullPointerException e)
