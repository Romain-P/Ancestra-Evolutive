...
public class Trunk {
	...
	public void HopIn(Personnage P)//Ouvrir coffre
	{
		...
		{
			SocketManager.GAME_SEND_MESSAGE(P, "Ce coffre ne peut être ouvert que par les membres de la guilde !", Server.config.getMotdColor());
		return;
		}
	...
	public void addInTrunk(int guid, int qua, Personnage P)
	{
		...
		{
			SocketManager.GAME_SEND_MESSAGE(P, "Le nombre d'objets maximal de ce coffre à été atteint !", Server.config.getMotdColor());
			return;
		}
		...
		{
			Log.addToLog("Le joueur "+P.get_name()+" a tenter d'ajouter un objet dans un coffre qu'il n'avait pas.");
			return;
		}
	...
	public void removeFromTrunk(int guid, int qua, Personnage P)
	{
		...
		{
			Log.addToLog("Le joueur "+P.get_name()+" a tenter de retirer un objet dans un coffre qu'il n'avait pas.");
			return;
		}
