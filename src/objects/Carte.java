...
public class Carte {
	...
	public static class MountPark
	{
		...
		public MountPark(int owner, Carte map, int cellid, int size,String data, int guild,int price)
		{
			...
			_size = size;
			_guild = World.data.getGuild(guild);
			_map = map;
			_cellid = cellid;
			_price = price;
			...
			for(String firstCut : data.split(";"))//PosseseurID,DragoID;PosseseurID2,DragoID2;PosseseurID,DragoID3
			{
				try
				{
					...
					Dragodinde DD = World.data.getDragoByID(Integer.parseInt(secondCut[1]));
					if(DD == null) continue;
		...
		public String parseData(int PID, boolean isPublic)
		{
			...
			for(Entry<Integer, Integer> MPdata : MountParkDATA.entrySet())
			{
				if(MPdata.getValue() == PID && isPublic)//Montrer que ses montures uniquement en public
				{
					...
					packet.append(World.data.getDragoByID(MPdata.getKey()).parse());
				}else
				{
					if(packet.length() > 0)packet.append(";");
					packet.append(World.data.getDragoByID(MPdata.getKey()).parse());
		...
		public String parseDBData()
		{
			...
			for(Entry<Integer, Integer> MPdata : MountParkDATA.entrySet())
			{
				...
				str.append(MPdata.getValue()).append(",").append(World.data.getDragoByID(MPdata.getKey()).get_id());
			}
			return str.toString();
		}
		...
		{
			for(Entry<Short, objects.Carte.MountPark> mp : World.data.getMountPark().entrySet())//Pour chaque enclo si ils en ont plusieurs
			{
				if(mp.getValue().get_guild().get_id() == GuildID)
				{
					if(!mp.getValue().getData().isEmpty())
					{
						...
						{
							Dragodinde d = World.data.getDragoByID(MPdata.getKey());
	...
	public static class InteractiveObject
	{
		...
		public InteractiveObject(Carte a_map,Case a_cell,int a_id)
		{
			...
			_template = World.data.getIOTemplate(_id);
		...
		public boolean isWalkable()
		{
			...
			return _template.getRespawnTime() <= 0 ? _template.isWalkable() : true;// && _state == Constants.IOBJECT_STATE_FULL;
		}
	}
	...
	public static class Case
	{
		...
		public boolean isWalkable(boolean useObject)
		{
			if(_object != null && useObject)
				return _Walkable && _object.isWalkable();// && _object.isWalkable();
			return _Walkable;
		}
		...
		public void startAction(Personnage perso, GameAction GA)
		{
			...
			switch(actionID)
			{
                ...
                case 104://Ouvrir coffre privé
                	...
                	if(trunk == null)
                    {
                    	Log.addToLog("Game: INVALID TRUNK ON MAP : "+perso.get_curCarte().get_id()+" CELLID : "+CcellID);
                    	return;
                    }
	...
	public Carte(short _id, String _date, byte _w, byte _h, String _key, String places, String dData,String cellsData,String monsters,String mapPos,byte maxGroup,byte maxSize)
	{
		...
		try
		{
			...
			_subArea = World.data.getSubArea(subArea);
			if(_subArea != null)_subArea.addMap(this);
		}catch(Exception e)
		{
			Log.addToLog("Erreur de chargement de la map "+_id+": Le champ MapPos est invalide");
			System.exit(0);
		}
		...
		for(String mob : monsters.split("\\|"))
		{
			...
			if(World.data.getMonstre(id) == null)continue;
			if(World.data.getMonstre(id).getGradeByLevel(lvl) == null)continue;
			_mobPossibles.add(World.data.getMonstre(id).getGradeByLevel(lvl));
		}
	...
	{
		NPC_tmpl temp = World.data.getNPCTemplate(npcID);
	...
	public void spawnGroup(int align, int nbr,boolean log,int cellID)
	{
		...
		for(int a = 1; a<=nbr;a++)
		{
			...
			if(log)
			{
				Log.addToLog("Groupe de monstres ajoutes sur la map: "+_id+" alignement: "+align+" ID: "+_nextObjectID);
	...
	public void spawnNewGroup(boolean timer,int cellID,String groupData,String condition)
	{
		...
		if(Server.config.isDebug()) Log.addToLog("Groupe de monstres ajoutes sur la map: "+_id+" ID: "+_nextObjectID);
	...
	public Fight newFight(Personnage init1,Personnage init2,int type, boolean targetProtected)
	{
		...
		Fight f = new Fight(type,id,this,init1,init2, targetProtected);
		_fights.put(id,f);
	...
	public int getRandomFreeCellID()
	{
		...
		if(freecell.isEmpty())
		{
			Log.addToLog("Aucune cellulle libre n'a ete trouve sur la map "+_id+" : groupe non spawn");
			return -1;
		}
	...
	public void onPlayerArriveOnCell(Personnage perso,int caseID)
	{
		...
		for(MobGroup group : _mobGroups.values())
		{
			if(Pathfinding.getDistanceBetween(this,caseID,group.getCellID()) <= group.getAggroDistance())//S'il y aggro
			{
				...
				{
					Log.addToLog(perso.get_name()+" lance un combat contre le groupe "+group.getID()+" sur la map "+_id);
	...
	{
		return (World.data.getSeller(get_id()) == null?0:World.data.getSeller(get_id()).size());
	}
}
