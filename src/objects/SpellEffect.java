...
public class SpellEffect
	{
				...
				for(SpellEffect buff : target.getBuffsByEffectID(id))
				{
					switch(id)
					{
						case 9://Derobade
							...
							int d = Pathfinding.getDistanceBetween(fight.get_map(), target.get_fightCell(false).getID(), caster.get_fightCell(false).getID());
							if(d >1)continue;
							...
							int exCase = target.get_fightCell(false).getID();
							int newCellID = Pathfinding.newCaseAfterPush(fight.get_map(), caster.get_fightCell(false), target.get_fightCell(false), nbrCase);
							if(newCellID <0)//S'il a été bloqué
							{
								int a = -newCellID;
								a = nbrCase-a;
								newCellID =	Pathfinding.newCaseAfterPush(fight.get_map(),caster.get_fightCell(false),target.get_fightCell(false),a);
								if(newCellID == 0)
									continue;
								if(fight.get_map().getCase(newCellID) == null)
									continue;
							}
							target.get_fightCell(false).getFighters().clear();
						...
						default:
							Log.addToLog("Effect id "+id+" definie comme ON_HIT_BUFF mais n'a pas d'effet definie dans ce gestionnaire.");
						break;
					}
				}
			}
		...
		public void applyToFight(Fight fight,Fighter acaster, ArrayList<Fighter> cibles, boolean isCaC)
		{
			...
			switch(effectID)
			{
				...
				default:
					Log.addToLog("effet non implante : "+effectID+" args: "+args);
				break;
			}
		}
		...
		private void applyEffect_50(Fight fight)
		{
			...
			target.set_fightCell(caster.get_fightCell(false));
		...
		private void applyEffect_185(Fight fight)
		{
			...
			try{
				...
				MG = World.data.getMonstre(mobID).getGradeByLevel(level).getCopy();
			}catch(Exception e1){
				Log.addToLog("Erreur sur le monstre id:"+mobID);
				return;
			};
		...
		private void applyEffect_783(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			Case ccase = caster.get_fightCell(false);
		...
		private void applyEffect_8(ArrayList<Fighter> cibles, Fight fight)
		{
			...
			Case exTarget = target.get_fightCell(false);
			Case exCaster = caster.get_fightCell(false);
		...
		private void applyEffect_402(Fight fight)
		{
			...
			SortStats TS = World.data.getSort(spellID).getStatsByLevel(level);
		...
		private void applyEffect_765B(Fight fight,Fighter target)
		{
			...
			Case cell1 = sacrified.get_fightCell(false);
			Case cell2 = target.get_fightCell(false);
			
			sacrified.get_fightCell(false).getFighters().clear();
			target.get_fightCell(false).getFighters().clear();
			sacrified.set_fightCell(cell2);
			sacrified.get_fightCell(false).addFighter(sacrified);
		...
		private void applyEffect_6(ArrayList<Fighter> cibles,Fight fight)
		{
			if(turns <= 0)
			{
				for(Fighter target : cibles)
				{
					...
					Case eCell = cell;
					//Si meme case
					if(target.get_fightCell(false).getID() == cell.getID())
					{
						//on prend la cellule caster
						eCell = caster.get_fightCell(false);
					}
					int newCellID =	Pathfinding.newCaseAfterPush(fight.get_map(),eCell,target.get_fightCell(false),-value);
					if(newCellID == 0)
						return;
		...
		private void applyEffect_141(final Fight fight,ArrayList<Fighter> cibles)
		{
			for(Fighter target : cibles)
			{
				if(target.hasBuff(765))//sacrifice
