...
public class Guild {
	...
	public static class GuildMember
	{
		...
		{
			return World.data.getPersonnage(_guid);
		}
	...
	{
		_id = World.data.getNextHighestGuildID();
		_name = name;
		_emblem = emblem;
		_lvl = 1;
		_xp= 0;
	...
	public void boostSpell(int ID)
	{
		...
		Spells.put(ID, ((SS == null)?World.data.getSort(ID).getStatsByLevel(1):World.data.getSort(ID).getStatsByLevel(SS.getLevel()+1)));
	}
	...
	public void removeMember(Personnage perso)
	{
		...
		if(h != null)
		{
			...
			{
				World.database.getHouseData().update(h, 0, 0);//On retire de la guilde
			}
		}
		_members.remove(perso.get_GUID());
		World.database.getGuildMemberData().delete(perso.getGuildMember());
	}
	...
	public void addXp(long xp)
	{
		...
		while(_xp >= World.data.getGuildXpMax(_lvl) && _lvl<200)
			levelUp();
	}
	...
	public void decompileSpell(String spellStr) //ID;lvl|ID;lvl|...
	{
		...
		for(String split : spellStr.split("\\|"))
		{
			...
			Spells.put(id, World.data.getSort(id).getStatsByLevel(lvl));
		}
	}
