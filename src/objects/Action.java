		...
		switch(ID)
		{
			...
			case 12://Spawn d'un groupe de monstre
				try
				{
					...
					if(inArena && !World.data.isArenaMap(perso.get_curCarte().get_id()))return;	//Si la map du personnage n'est pas classé comme étant dans l'arène
			...
			case 101://Arriver sur case de mariage
				...
				{
					World.data.AddMarried(perso.get_sexe(), perso);
				}else 
				{
			...
			case 102://Marier des personnages
				World.data.PriestRequest(perso, perso.get_curCarte(), perso.get_isTalkingWith());
			break;
			case 103://Divorce
			...
			case 228://Faire animation Hors Combat
				try
				{
					...
					Animations animation = World.data.getAnimation(AnimationId);
			...
			break;
			default:
				Log.addToLog("Action ID="+ID+" non implantee");
			break;
		}
	}
