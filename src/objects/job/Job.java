...
public class Job {
	...
	public static class StatsMetier
	{
		...
		public void endAction(int id,Personnage P,InteractiveObject IO,GameAction GA,Case cell)
		{
			...
			addXp(P,_curAction.getXpWin()*Server.config.getRateXpJob());
			//Packet JX
			//on créer la listes des statsMetier a envoyer (Seulement celle ci)
			ArrayList<StatsMetier> list = new ArrayList<StatsMetier>();
			list.add(this);
		...
		public void addXp(Personnage P,long xp)
		{
			...
			while(_xp >= World.data.getExpLevel(_lvl+1).metier && _lvl <100)
				levelUp(P,false);
		...
		{
			StringBuilder str = new StringBuilder();
			str.append( World.data.getExpLevel(_lvl).metier).append(s);
			str.append(_xp).append(s);
			str.append(World.data.getExpLevel((_lvl<100?_lvl+1:_lvl)).metier);
			return str.toString();
		}
	...
	public static class JobAction
	{
		...
		public JobAction(int sk,int min, int max,boolean craft, int arg,int xpWin)
		{
			...
			_craftTimer = new Timer(850,new ActionListener()
			{
		...
		public void craft()
		{
			...
			for(Entry<Integer,Integer> e : _ingredients.entrySet())
			{
				...
				if(!_P.hasItemGuid(e.getKey()))
				{
					...
					Log.addToLog("/!\\ "+_P.get_name()+" essaye de crafter avec un objet qu'il n'a pas");
					return;
				}
				...
				{
					SocketManager.GAME_SEND_Ec_PACKET(_P,"EI");
					Log.addToLog("/!\\ "+_P.get_name()+" essaye de crafter avec un objet qui n'existe pas");
					return;
				}
				...
				{
					SocketManager.GAME_SEND_Ec_PACKET(_P,"EI");
					Log.addToLog("/!\\ "+_P.get_name()+" essaye de crafter avec un objet dont la quantite est trop faible");
					return;
				}
			...
			int tID = World.data.getObjectByIngredientForJob(SM.getTemplate().getListBySkill(_skID),items);
			...
			//On donne l'xp
			int winXP =  Constants.calculXpWinCraft(SM.get_lvl(),_ingredients.size()) * Server.config.getRateXpJob();
			if(success)
			{
				SM.addXp(_P,winXP);
		...
		private void doFmCraft()
		{
			...
			{
				Objet ing = World.data.getObjet(guid);
			...
			}else
			{
				...
				else if(isStatsChanging > 0 && isElementChanging == 0)//Si on modifier les stats (rune)
				{
					...
					{
						if(Server.config.isDebug()) Console.instance.println("Modification d'un stat existant : "+stats+". Ajout de "+add);
					...
					{
						if(Server.config.isDebug()) Console.instance.println("Ajout d'un stat inexistant : "+stats+". Ajout de "+add); 
				...
				World.database.getItemData().update(obj);//On Save
				//TODO : Le repeat ?
		}
			...
			if(sign != null)
			{
				...
				if(newQua <= 0)
				{
					...
					World.data.removeItem(sign.getGuid());
			...
			if(mod != null)
			{
				...
				if(newQua <= 0)
				{
					...
					World.data.removeItem(mod.getGuid());
		...
		public void putLastCraftIngredients()
		{
			...
			for(Entry<Integer,Integer> e : _ingredients.entrySet())
			{
				if(World.data.getObjet(e.getKey()) == null)return;
				if(World.data.getObjet(e.getKey()).getQuantity() < e.getValue())return;
