...
public class Objet {
	...
	public static class ObjTemplate
	{
		...
		{		
			Objet item = new Objet(World.database.getItemData().nextId(), ID, qua, Constants.ITEM_POS_NO_EQUIPED, generateNewStatsFromTemplate(StrTemplate,useMax),getEffectTemplate(StrTemplate));
			return item;
		}
	...
	{
		this.guid = Guid;
		this.template = World.data.getObjTemplate(template);
		this.quantity = qua;
		this.position = pos;
	...
	{
		Objet ob = new Objet(World.data.getNewItemGuid(), obj.getTemplate().getID(), qua,Constants.ITEM_POS_NO_EQUIPED, obj.getStats(), obj.getEffects());
		return ob;
	}
