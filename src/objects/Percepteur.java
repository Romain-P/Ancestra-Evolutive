...
public class Percepteur
{
	...
	public void DelPerco(int percoGuid)
	{
		for(Objet obj : _objets.values())
		{
			...
			World.data.removeItem(obj.guid);
		}
	...
	public static String parsetoGuild(int GuildID)
	{
		...
		for(Entry<Integer, Percepteur> perco : World.data.getPercos().entrySet())
		{
    		 ...
    		 {
				 	Carte map = World.data.getCarte((short)perco.getValue().get_mapID());
	...
	public static String parseAttaqueToGuild(int guid, short mapid, int fightid)
	{
		...
		for(Entry<Integer, Fight> F : World.data.getCarte(mapid).get_fights().entrySet())
		{
