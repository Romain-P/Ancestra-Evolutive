...
public class Commands {
	...
	private void fullHdv(int ofEachTemplate)
	{
		...
		for (ObjTemplate curTemp : World.data.getObjTemplates())//Boucler dans les template
		{
			try
			{
				if(Server.config.getNoInHdv().contains(curTemp.getID())) continue;
				for (int j = 0; j < ofEachTemplate; j++)//Ajouter plusieur fois le template
				{
					...
					World.data.getHdv(hdv).addEntry(entry);
					World.data.addObjet(objet, false);
				}
			}catch (Exception e)
			{
				continue;
		...
		SocketManager.GAME_SEND_MESSAGE_TO_ALL("HDV remplis!",Server.config.getMotdColor());
	}
