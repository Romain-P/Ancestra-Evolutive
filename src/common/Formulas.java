...
public class Formulas {
	...
	public static int calculFinalDommage(Fight fight,Fighter caster,Fighter target,int statID,int jet,boolean isHeal, boolean isCaC, int spellid)
	{
		...
		if(statC<0)statC=0;
		/* DEBUG
		Console.instance.println("Jet: "+jet+" Stats: "+statC+" perdomC: "+perdomC+" multiplier: "+multiplier);
		Console.instance.println("(100 + statC + perdomC)= "+(100 + statC + perdomC));
		Console.instance.println("(jet * (100 + statC + perdomC + (multiplier*100) ) / 100)= "+(jet * ((100 + statC + perdomC) / 100 )));
		Console.instance.println("res Fix. T "+ resfT);
		Console.instance.println("res %age T "+respT);
		if(target.getMob() != null)
		{
			Console.instance.println("resmonstre: "+target.getMob().getStats().getEffect(Constants.STATS_ADD_RP_FEU));
			Console.instance.println("TotalStat: "+target.getTotalStats().getEffect(Constants.STATS_ADD_RP_FEU));
			Console.instance.println("FightStat: "+target.getTotalStatsLessBuff().getEffect(Constants.STATS_ADD_RP_FEU));
