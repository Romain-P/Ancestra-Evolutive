...
public class SocketManager {
	...
	public static void GAME_SEND_ASK(Client out,Personnage perso)
	{
		StringBuilder packet = new StringBuilder();
	...
	{
		String packet = "cMK"+suffix+"|"+guid+"|"+name+"|"+msg;
		for(Personnage perso : World.data.getOnlinePersos())if(perso.isOnline())if(perso.get_compte() != null)if(perso.get_compte().get_gmLvl()>0)send(perso,packet);
		if(Server.config.isDebug())
			Log.addToSockLog("Game: ALL("+World.data.getOnlinePersos().size()+"): Send>>"+packet);
	}
	...
	public static void GAME_SEND_WEDDING(Carte c, int action, int homme, int femme, int parlant)
	{
		...
		Personnage Homme = World.data.getPersonnage(homme);
		send(Homme,packet);
		if(Server.config.isDebug())
			Log.addToSockLog("Game: Send>>" + packet);
	} 
