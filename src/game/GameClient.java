...
public class GameClient implements Client {
	...
	private void guild_promote(String packet)
	{
		...
		Personnage p = World.data.getPersonnage(guid);	//Cherche le personnage a qui l'on change les droits dans la m�moire
	...
	private void guild_kick(String name)
	{
		...
		else
		{
			...
			{
					toRemGuild = World.data.getGuild(_perso.get_guild().get_id());//On prend la guilde du perso qui l'�jecte
			}
	...
	private void Basic_chatMessage(String packet)
	{
		...
		switch(packet.charAt(2))
		{
			...
			case ':'://Canal commerce
				...
				{
					l = (Server.config.getFloodTime()  - l)/1000;//On calcul la diff�rence en secondes
			...
			case '?'://Canal recrutement
				...
				{
					j = (Server.config.getFloodTime()  - j)/1000;//On calcul la diff�rence en secondes
			...
			case '!'://Alignement
				...
				{
					k = (Server.config.getFloodTime()  - k)/1000;//On calcul la diff�rence en secondes
